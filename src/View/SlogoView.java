...
public class SlogoView {
    ...
    public SlogoView () throws ClassNotFoundException {
        ...
        myGridFactory = new GridFactory(DEFAULT_SIZE.height - 100, DEFAULT_SIZE.width - 200,
                this.build(5), null);
     ...
     * 
     * @param mainStage
     *            the Stage for the GUI to operate on
    ...
    /**
     * Passes the commands from the textbox on the GUI to the controller for
     * parsing.
     * 
     * @param s
     *            String representing the command to send that was inputed by
     *            the user
     * 
     */
    private void sendCommand () {
        if (commandLine.getText() != null) {
            ...
            ButtonTemplate mostRecent = new ButtonTemplate(commandLine.getText(), 0, 0, null, 180,
                    35);
    ...
    // ugly workaround, need to find elegant solution to get rid of repeated
    // code
    ...
    private void createMenuItems (MenuTemplate pMenu, String resource,
            HashMap<String, GUIFunction> myMap) {
        /*
         * ResourceBundle personalizeItems=ResourceBundle.getBundle(
         * "/resources/Personalize.Properties"); for (String s: myMap.keySet()){
         * pMenu.addMenuItem(personalizeItems.getString(s),
         * event->myMap.get(s).doAction()); }
         */
    ...
    private void createMenuItemsUnderFile (MenuTemplate fileMenu) throws ClassNotFoundException,
    ...
    private void addTurtle () {
        // myController.addTurtle(myGrids.getActiveGrid().addTurtle(),
        // myGrids.getActiveGrid().getID(), true);
    ...
    private Pane setTextArea () {
        ...
        myTextArea
                .setStyle("-fx-background-color: #000080; -fx-border-color: BLACK; -fx-border-width: 5");
        ...
        /*
         * ColorBar backGroundColorBar=new
         * ColorBar(myResources.getString("backgroundColor"));
         * backGroundColorBar.relocate(25, 350); ColorBar penColorBar=new
         * ColorBar(myResources.getString("penColor"));
         * backGroundColorBar.relocate(25, 350); penColorBar.relocate(25, 385);
         * Iterator
         * <String>myIterator=colorSelection.getAvailableColors().iterator();
         * while (myIterator.hasNext()){ String color=(String)myIterator.next();
         * ColorFunction myFunction= (ColorFunction)
         * myUserFunctions.get("backgroundColor");
         * backGroundColorBar.addItem(color, event->myFunction.doAction(color));
         * ColorFunction myPenFunction=(ColorFunction)
         * myUserFunctions.get("penColor"); penColorBar.addItem(color,
         * event->myPenFunction.doAction(color)); }
         */
        ...
        myTextArea.getChildren().addAll(label, commandLine, enter, makeCommand, history,
                commandHistoryBox, colorSelection);
        return myTextArea;
    }
        ...
        try {
            ...
            for (String s : myUserFunctions.keySet()) {
                if (myUserFunctions.get(s).getClass().getSuperclass().equals(myClass)) {
                    ...
                    myButtons.add(new ButtonTemplate(value[0], Double.parseDouble(value[1]), Double
                            .parseDouble(value[2]), myUserFunctions.get(s)));
                }
            }
            return myButtons;
        } catch (IOException e) {
    ...
    private void makeMenu (Class myClass, MenuTemplate myMenu) {
        for (String s : myUserFunctions.keySet()) {
            Class<? extends GUIFunction> myFunctionClass = (Class<? extends GUIFunction>) myUserFunctions
                    .get(s).getClass().getSuperclass();
    ...
    private Collection<ScrollingBar> makeSlidingBars () {
        ...
        ScrollingBar myPenBar = new PenScrollingBar(myResources.getString("penThickness"), 200, 20,
                myUserFunctions.get("penThickness"));
    ...
    private void makeListOfFunctions () {
        ...
        // myUserFunctions.put(myResources.getString("spanish"), new
        // SetLanguage());
