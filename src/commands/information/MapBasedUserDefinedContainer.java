...
/**
 * Map based implementation of BaseUserDefinedContainer
 * 
 * @author Duke
 *
 */
public class MapBasedUserDefinedContainer extends BaseUserDefinedContainer {
	...
	@Override
	public boolean addVariable(String variable, BaseCommand command)
			throws BackendException {
		if (command == null || variable == null || variable.equals("")) {
			throw new BackendException(null,
					"Invalid variable or command provided");
		}
		try {
			...
			throw new BackendException(null,
					"Unexpected error when trying to set variable");
		}
	}

	@Override
	public boolean addVariable(String variable, double value)
			throws BackendException {
		NumericalCommand number = new NumericalCommand(
				((Double) value).toString(), true);
	...
	@Override
	public void addNewCommand(String commandName, String innerCommands,
	...
	public BaseCommand getCreatedCommand(String commandName, String input,
		...
		for (int i = 0; i < variables.length; i++) {
			BaseCommand expression = CommandFactory.createCommand(subInput,
					true);
			subInput = expression.getLeftoverString();
			correspondingExpressions[i] = expression;
		}

		String innerCommandString = new String(
				myCreatedCommandMap.get(commandName));
		BaseCommand innerCommand = CommandFactory.createCommand(
				innerCommandString, false);

		CreatedCommand createdCommand = new CreatedCommand(subInput,
				isExpression);
		createdCommand.setInnerCommand(innerCommand);
		createdCommand
				.setVariables(variables.clone(), correspondingExpressions);
		return createdCommand;
	}

	@Override
	public Map<String, Double> getAllVariablesAndValues()
			throws BackendException {
		...
		for (String variable : myVariableToValuesMap.keySet()) {
			...
			try {
				...
				throw new BackendException(e,
						"There is an invalid variable in the container");
			}
		}
