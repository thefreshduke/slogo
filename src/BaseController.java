...
/**
 * The controller which acts as the gateway between the View and the Model. All communication
 * between the two will go through the controller. The controller will send to the controller text
 * commands that the end user enters. After parsing by the parser, the controller will either send
 * the command back to the view or the model, depending on what kind of command it is. 
 * If a command changes the turtle's state which needs to be visualized, that information will be
 * sent to the view from the model via the controller (i.e. new location, orientation). 
 * 
 * @author Duke
 *
 */
